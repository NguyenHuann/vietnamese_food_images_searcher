...
export default function DishDetail() {
  ...
  const description =
    ...
    `${dish_name} là một trong những món ăn truyền thống nổi tiếng của Việt Nam, mang đậm hương vị và bản sắc văn hóa ẩm thực dân tộc.`;

  const similarDishes = allResults
    .filter((r) => r.image_url !== image_url)
    .slice(0, 4);
        ...
        <div className="w-full h-[350px] md:h-[600px] rounded-[1.5rem] md:rounded-[2rem] overflow-hidden relative shadow-2xl bg-surface-container-highest">
          ...
          <div className="absolute bottom-6 left-6 right-6 md:bottom-12 md:left-12 md:right-12 flex flex-col md:flex-row items-start md:items-end justify-between gap-6 md:gap-8">
            <div className="space-y-2 md:space-y-4">
              <span className="micro-label !text-white/60 !mb-0">
                Khám phá chi tiết
              </span>
              <h1 className="text-4xl md:text-8xl font-bold text-white tracking-tighter drop-shadow-sm leading-none">
                {dish_name}
              </h1>
              <span
                className={`inline-block px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest ${similarity >= 0.8 ? "bg-green-500/80 text-white" : similarity >= 0.5 ? "bg-amber-500/80 text-white" : "bg-white/20 text-white"}`}
              >
            ...
            <button
              onClick={() =>
                toggleFavorite({ dish_name, image_url, timestamp: Date.now() })
              }
        ...
        <div className="lg:col-span-7 space-y-12 md:space-y-16">
          ...
          <section className="space-y-6 md:space-y-8">
            ...
            <div className="space-y-4 md:space-y-6 text-lg md:text-xl text-on-surface-variant leading-relaxed font-body">
              ...
              <p>
                Mỗi vùng miền lại có cách chế biến riêng, tạo nên sự đa dạng và
                phong phú cho nền ẩm thực Việt.
              </p>
            </div>
          </section>
                ...
                {ingredients.map((ing, i) => (
                  ...
                  >
                    <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-3xl border border-outline/5 text-center space-y-3 md:space-y-4 shadow-sm hover:shadow-xl transition-all h-full cursor-help">
                      ...
                      <p className="font-bold text-lg md:text-xl text-on-surface leading-snug">
                        {ing.name}
                      </p>
                    </div>
                ...
                {recipeSteps.map((item) => (
                  ...
                  >
                    ...
                    <div className="space-y-2 md:space-y-3 pt-1 md:pt-2">
                      <h4 className="text-xl md:text-2xl font-bold text-on-surface group-hover:text-primary transition-colors">
                        {item.title}
                      </h4>
              ...
              <div className="py-24 text-center space-y-6 relative z-10">
                ...
                <div className="space-y-2">
                  <p className="text-xl font-serif italic text-on-surface-variant">
                    Công thức chế biến sẽ được cập nhật sớm.
                  </p>
        ...
        <div className="lg:col-span-5 space-y-16">
            ...
            <div className="space-y-10">
              <div className="flex justify-between items-end">
                ...
                <span className="text-5xl font-bold text-on-surface leading-none">
                  ...
                  <span className="text-sm font-normal text-on-surface-variant uppercase tracking-widest ml-1">
                    %
                  </span>
                </span>
              </div>
              <div className="h-3 bg-surface-container-highest rounded-full overflow-hidden">
                <motion.div
                  ...
                  className={`h-full rounded-full ${similarity >= 0.8 ? "bg-green-500" : similarity >= 0.5 ? "bg-amber-500" : "bg-red-400"}`}
                />
              </div>
              <p
                className={`text-center text-lg font-serif italic ${similarity >= 0.8 ? "text-green-600" : similarity >= 0.5 ? "text-amber-600" : "text-red-500"}`}
              >
                {similarity >= 0.8
                  ? "Rất chính xác"
                  : similarity >= 0.5
            ...
            <section className="bg-surface-container-highest rounded-[3rem] overflow-hidden border border-outline/10 p-8 md:p-12 space-y-8">
              ...
              <div className="space-y-4">
                {similarDishes.map((dish, i) => (
                  ...
                  >
                    <div
                      className={`w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 shadow-sm border-2
                    ...
                    <div className="flex-grow min-w-0">
                      ...
                      <p className="text-sm text-on-surface-variant">
                        {(dish.similarity * 100).toFixed(1)}%
                      </p>
                    </div>
                    <ArrowRight
                      size={16}
                      className="text-on-surface-variant/30 group-hover:text-primary transition-colors flex-shrink-0"
                    />
                  </Link>
                ))}
              </div>
          ...
          <section className="flex flex-col h-fit bg-surface-container-highest rounded-[3rem] overflow-hidden shadow-2xl group border border-outline/10">
            ...
            <div className="p-12 space-y-8">
              <div className="space-y-4">
                <h4 className="text-3xl font-bold italic tracking-tight">
                  Tìm quán {dish_name}
                </h4>
                <p className="text-on-surface-variant text-lg leading-relaxed font-body">
                  Tìm kiếm các địa điểm phục vụ {dish_name} quanh khu vực của
                  bạn.
                </p>
              </div>
              <a
                href={`https://www.google.com/search?q=quán+${encodeURIComponent(dish_name)}+gần+đây`}
                target="_blank"
